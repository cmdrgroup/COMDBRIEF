          ...
          {showAddForm && (
            <div className="directive-card mb-6 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <input value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="Email" className={`w-full bg-background border rounded-sm px-3 py-2 text-sm text-steel-white focus:outline-none focus:border-command-gold ${newEmail && !isValidEmail(newEmail) ? "border-warning-red" : "border-gunmetal"}`} />
                  {newEmail && !isValidEmail(newEmail) && <p className="text-warning-red text-[10px] mt-1 font-mono">Invalid email format</p>}
                </div>
              </div>
              <button
